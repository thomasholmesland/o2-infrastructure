# Publiserte porter

> Automatisk generert fra `o2-core`  
> Generert: `2026-07-28T04:44:31+00:00`  
> Server-IP: `192.168.1.241`

Denne filen skal ikke redigeres manuelt.

| Container | Vert-IP | Vert-port | Containerport | Protokoll |
|---|---|---:|---:|---|
| `/cadvisor` | `0.0.0.0` | `8080` | `8080` | `tcp` |
| `/cadvisor` | `::` | `8080` | `8080` | `tcp` |
| `/grafana` | `0.0.0.0` | `3000` | `3000` | `tcp` |
| `/grafana` | `::` | `3000` | `3000` | `tcp` |
| `/nginx-proxy-manager` | `0.0.0.0` | `443` | `443` | `tcp` |
| `/nginx-proxy-manager` | `::` | `443` | `443` | `tcp` |
| `/nginx-proxy-manager` | `0.0.0.0` | `80` | `80` | `tcp` |
| `/nginx-proxy-manager` | `::` | `80` | `80` | `tcp` |
| `/nginx-proxy-manager` | `0.0.0.0` | `81` | `81` | `tcp` |
| `/nginx-proxy-manager` | `::` | `81` | `81` | `tcp` |
| `/node-exporter` | `0.0.0.0` | `9100` | `9100` | `tcp` |
| `/node-exporter` | `::` | `9100` | `9100` | `tcp` |
| `/portainer` | `0.0.0.0` | `9443` | `9443` | `tcp` |
| `/portainer` | `::` | `9443` | `9443` | `tcp` |
| `/prometheus` | `0.0.0.0` | `9090` | `9090` | `tcp` |
| `/prometheus` | `::` | `9090` | `9090` | `tcp` |
| `/uptime-kuma` | `0.0.0.0` | `3001` | `3001` | `tcp` |
| `/uptime-kuma` | `::` | `3001` | `3001` | `tcp` |
